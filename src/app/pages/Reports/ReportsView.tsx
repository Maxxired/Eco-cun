...
const ReportsView: React.FC = () => {
    ...
    const fetchReports = async () => {
      try {
        ...
        if (fullResponse && fullResponse.data) {
          const translatedReports = fullResponse.data.map(
            (report: ReportFromApi): ReportCardProps => ({
              folio: report.id.toString().padStart(4, "0"),
              ubicacion: `Lat: ${report.locLatitude.toFixed(
                4
              )}, Lon: ${report.locLongitude.toFixed(4)}`,
  ...
  return (
    <div className="min-h-screen bg-white pb-20 flex flex-col">
      ...
      {/* Footer del Monito */}
      <div className="text-center space-y-4 pt-8 pb-4"></div>
    </div>
  );
};
