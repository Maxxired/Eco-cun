...
export const GamificationSection: React.FC<GamificationProps> = ({
  ...
  useEffect(() => {
    const fetchLevelsData = async () => {
      try {
        const response = await fetch(`${API_URL}/api/gamification/levels`, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });

        if (!response.ok) throw new Error("Error al obtener niveles");

        const data: BackendLevel[] = await response.json();
        setLevels(data);
      } catch (err) {
        console.error(err);
      }
  ...
  const LEVELS = useMemo(() => {
    return LEVEL_META.map((meta) => {
      const backend = levels.find((l) => l.name === meta.name);

      return {
        ...meta,
        minPoints: backend?.minPoints ?? 0,
        maxPoints: backend?.maxPoints ?? 0,
        multiplier: backend?.multiplier ?? 1,
      };
    });
  }, [levels]);

  const currentLevelDetails = LEVELS.find(
    (lvl) => points >= lvl.minPoints && points <= lvl.maxPoints
  );
  const currentLevel = currentLevelDetails ?? LEVELS[0];
  const currentLevelIndex = LEVELS.findIndex(
    (lvl) => lvl.name === currentLevel.name
  );
  const nextLevel = LEVELS[currentLevelIndex + 1] ?? null;

  const getLevelProgress = (points: number, min: number, max: number) => {
  ...
  const pointsToNextLevel = useMemo(() => {
    ...
    const remaining = currentLevel.maxPoints - points;
    return remaining > 0 ? remaining : 0;
  }, [currentLevel, points]);

  const progress = currentLevelDetails
    ? getLevelProgress(
        points,
        currentLevelDetails.minPoints,
        currentLevelDetails.maxPoints
      )
    : 0;
  ...
  return (
    <div className="bg-white rounded-2xl shadow-md p-6 mb-6 border border-gray-100 relative overflow-hidden">
      <div
        className={`absolute top-0 right-0 w-32 h-32 rounded-bl-full opacity-10 ${currentLevel.bg}`}
      ></div>

      {/* --- ENCABEZADO: NIVEL ACTUAL --- */}
      <div className="flex items-center gap-4 mb-6 relative z-10">
        <div
          className={`w-14 h-14 rounded-full flex items-center justify-center text-3xl ${currentLevel.bg} ${currentLevel.color} shadow-sm`}
        >
          {currentLevel.icon}
        </div>
        <div>
      ...
      <div className="mb-6 relative z-10">
        <div className="flex justify-between text-xs text-gray-500 mb-1">
          ...
          {nextLevel ? (
            <span>
              Faltan <strong>{pointsToNextLevel} pts</strong>
            </span>
          ) : (
              ...
              >
                <div
                  className={`w-10 h-10 md:w-12 md:h-12 rounded-xl flex items-center justify-center text-lg md:text-xl transition-all duration-300
                    ${
                      isUnlocked
                        ? `bg-white border-2 border-${
                            level.color.split("-")[1]
                          }-200 shadow-md scale-105`
